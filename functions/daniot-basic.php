...
function daniot_head() {
    ...
    global $post;
    if ( is_home() ) {
        $keywords    = daniot_settings( 'keywords' );
        $description = daniot_settings( 'description' );
    } elseif ( is_single() ) {
        ...
        if ( $description == "" ) {
            if ( $post->post_excerpt ) {
                ...
                $description = daniot_excerpt( $post->post_content, 200 );
            }
        }
    } elseif ( is_page() ) {
        $keywords    = get_post_meta( $post->ID, "keywords", true );
    ...
    <link rel="Shortcut Icon" href="<?php echo daniot_settings( 'favicon' ); ?>" type="image/x-icon"/>
    <?php wp_head(); ?>
<?php
}
...
function daniot_settings( $key ) {
    ...
    $settings = get_option( daniot_NAME . '_settings' );
 ...
 * @return string
 */
function daniot_path( $path ) {
    $file_path = daniot_PATH . '/' . $path;
 ...
 * @return string
 */
function daniot_file_url( $file_path ) {
    $file_path = daniot_THEME_URL . '/' . $file_path;
 ...
 * @return string
 */
function daniot_style( $style_name ) {
    $style_url = daniot_file_url( 'public/css/' . $style_name );
 ...
 * @return string
 */
function daniot_script( $script_name ) {
    $script_url = daniot_file_url( 'public/js/' . $script_name );
 ...
 * @return string
 */
function daniot_image( $image_name ) {
    $image_url = daniot_file_url( 'public/images/' . $image_name );
 ...
 * @return string
 */
function daniot_excerpt( $content, $limit = 100 ) {
    if ( $content ) {
 ...
 * @return string
 */
function daniot_time_since( $older_date, $comment_date = false ) {
    $chunks = array(
        array( 24 * 60 * 60, __( ' days ago', daniot_NAME ) ),
        array( 60 * 60, __( ' hours ago', daniot_NAME ) ),
        array( 60, __( ' minutes ago', daniot_NAME ) ),
        array( 1, __( ' seconds ago', daniot_NAME ) )
    );
 ...
 * Post views
 */
function daniot_views() {
    if ( function_exists( 'the_views' ) ) {
        global $post;
        ?>
        <li class="inline-li"><span class="post-span">·</span></li>
        <li class="inline-li"><?php echo daniot_views_count() . __( ' views', daniot_NAME ); ?></li>
    <?php
    }
}
 ...
 * @return mixed|string
 */
function daniot_views_count( $post_id = null ) {
    global $post;
 ...
 * @return array
 */
function daniot_thumbnail( $type = 'full', $width = 0, $height = 0 ) {
    global $post;
    ...
    } else if ( ! daniot_settings( 'thumbnail' ) ) {
        ob_start();
        ob_end_clean();
 ...
 * @return string
 */
function daniot_cdn( $obj, $width = 0, $height = 0, $is_avatar = false ) {
    $url       = '';
    $need_crop = true;
    $cdn_type  = daniot_settings( 'cdn' );
    ...
    if ( $cdn_type == 0 ) {
        ...
        if ( $need_crop && !$is_avatar) {
            $url = sprintf( '%s&#63;src=%s&#38;w=%s&#38;h=%s&#38;zc=1&#38;q=100', daniot_file_url( 'timthumb.php' ), urlencode( $url ), $width, $height );
        }
 ...
 * @param int $space
 */
function daniot_pagenavi( $space = 5 ) {
    if ( is_singular() ) {
        return;
    }
